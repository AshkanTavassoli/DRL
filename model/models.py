...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    model_ensemble = [list() for i in range(10)]
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        if (sharpe_ppo >= sharpe_a2c) & (sharpe_ppo >= sharpe_ddpg):
            for j in range(7):
                model_ensemble[j] = model_ppo
            if (sharpe_a2c > sharpe_ddpg):
                for j in range(7,10):
                    model_ensemble[j] = model_a2c
            else:
                for j in range(7,10):
                    model_ensemble[j] = model_ddpg
            model_use.append('PPO')
        elif (sharpe_a2c > sharpe_ppo) & (sharpe_a2c > sharpe_ddpg):
            for j in range(7):
                model_ensemble[j] = model_a2c
            if (sharpe_ppo > sharpe_ddpg):
                for j in range(7,10):
                    model_ensemble[j] = model_ppo
            else:
                for j in range(7,10):
        ...
        for part_i in range(10):
            ...
            last_state_ensemble[part_i] = DRL_prediction(df=df, model=model_ensemble[part_i], name="ensemble",
